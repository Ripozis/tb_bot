...
def read_tbl():
        con = """SELECT * from (SELECT title_ru, url, max(likes) as likes, id_post from parser where path_file is not null and title_ru is not null and publication_attribute =0 and public_attr_dev is NULL and content_error is NULL and to_remove=0 and for_editing =0 and content_format is not NULL) LIMIT 1"""
        cur.execute(con)
